window.__ModuleLoader__.load({ id: 'dsh-windows-workspace-guard', factory: (require) => {
  ...
  const zh = {
    ...
    block: '阻断', ask: '请求一次批准', report: '仅记录', toolNames: '受保护工具名称（已验证：pwsh、read、read_image、write、edit、glob、grep、str_replace_editor）',
  ...
  const en = {
    ...
    block: 'Block', ask: 'Ask once', report: 'Report only', toolNames: 'Guarded tools (verified: pwsh, read, read_image, write, edit, glob, grep, str_replace_editor)',
  ...
  function draftOf(value = {}) {
    return {
      ...
      toolNames: lines(value.toolNames?.length ? value.toolNames : ['pwsh', 'read', 'read_image', 'write', 'edit', 'glob', 'grep', 'str_replace_editor']),
