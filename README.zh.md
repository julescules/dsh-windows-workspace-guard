...
在 DSH `v0.1.0-rc.7` 或更新版本中，也可以从“设置 → 插件 → Windows 工作区防护”修改这些字段，保存后立即生效，不需要重启插件。持久 Windows PowerShell 集成推荐使用 DSH `v0.1.0-rc.8` 或更新版本。
...
- 已通过真实 `@deepseek-ai/dsh@0.1.0-rc.8` Profile 安装、配置组合、Web Host 启动图发现及客户端 bundle 服务验证；
- 安装时不需要运行构建脚本；
- UTF-8 追加式审计及常见密钥脱敏。
